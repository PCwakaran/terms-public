1. 心得  
    ...
    参加者は大人として振る舞い、他の参加者を大人として扱うこと。
    
    1. **他人に敬意**を持って接すること。
    2. 今は知識がつき、理解している人も最初は初心者である。それを忘れず驕らず昂らず。
    3. 何かあっても穏便に対応すること。
    4. 怪しいファイルはダウンロードしない。
        ファイル送信者はファイルの中身を明記する等の配慮を行うこと。
    5. モラルハザードを起こさないこと。
...
2. 禁止行為
    ...
    2. 同じ内容の**連投**
    3. しつこい宣伝  
        荒らしサーバ等への勧誘は1度でも荒らし行為とみなし、後述の処罰対象となる可能性がある。
    4. しつこい**権限**の要求
    5. 日本国における各種法律に抵触する行為
    6. Discordの規約に抵触する行為
    7. **宗教勧誘**行為  
        新興宗教等の勧誘などを指し、ソフトウエアや名称等の宗教はこれに含まない。
    8. **反社会的勢力**に該当する行為  
        暴力団・カルト宗教等、反社会勢力にかかわる内容・勧誘。
    9. 企業や学校等の**機密事項**を公に発言する行為  
        守秘義務、インサイダー情報、関係者しか知り得ない情報の投稿等。
    10. 特定の人間をタコ殴りにする行為  
        **本人が嫌がっているにも関わらず** 、しつこく進捗を確認し続ける等。
    11. 危険行動を助長する行為
    12. **薬物乱用**の宣言・助長する行為  
        オーバードーズ、違法薬物に関する内容等。
    ...
    16. **他人の個人情報を公表**する行為  
        ...
        - Webサイトに公開されている
        - 行政資料等の請求で開示ができる
        - 本人が公言している場合 (この場合、「投稿している」必要がある。VCでの発言は証拠の検証が不可能であるため、これに含まない。)
        
        上記に関しては、情報のソースについて確認する場合がある。
            
        具体例
        - 他人の情報に関して「ここで知り得ない」情報の投稿。
            - リアル・他の場所での知り合いが 年齢を偽って18歳以上のroleを登録していると暴露する行為等。
        - 他人の写真の本人の意図に沿わない投稿。
        - 個人情報の本人の意図に沿わない投稿。
...
6. 入室  
    メンバーロールの付与は、**自己紹介 を 「アイサツ」チャンネルへ投稿後**、メンバーロールをモデレータが付与する。  
    暴力団、公安監視対象等の**反社会的勢力に属するものについては入室を禁ずる**
    
    下記のテンプレートに従い文章を作成し、挨拶「アイサツ」チャンネルに投稿すること。  
    テンプレートに従わないメッセージは原則認められない。
...
7. 権限  
    ...
    2. モデレータ (青)  
        以下の権限を保有し、**サーバの維持管理の協力**を行う。メンバーに追加し、設定を行う。
        - 発言の削除
        - ユーザ権限の操作
        - ユーザロールの操作
        - チャンネルの追加・削除
        ...
        - ご案内ちゃんねるへの発言
    3. 発言削除権限  
        以下の権限を保有し、**サーバ運営上必要な発言の削除**を行う。メンバーに追加し、設定を行う。
...
9. 依頼  
    チャンネルの追加、機能の追加、発言の削除、荒らしの対応等、何らかの要望がある場合は「要望・問い合わせ等」チャンネルに要望を送ることが可能である。
...
10. 処罰  
    次に該当する場合、モデレータが各種権限の剥奪・閲覧制限等を行う場合がある。
...
11. 荒らしの緊急対応  
    ...
    メンバーはモデレータに対しメンション機能を使い報告を行い、モデレータに対応を促すこと。
...
13. 例外  
    電話認証が必須となっているため、「アイサツ」チャンネルへ自己紹介を記載できない場合次の方法で回避することが可能である。
