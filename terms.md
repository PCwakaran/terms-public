1. 心得  
    ...
    1. **他人に敬意**を持って接する。
    2. 今は知識がついてわかっている人も最初は初心者、それを忘れず驕らず昂ぶらず。
    3. 何かあっても穏便に対応する。
    4. 怪しいファイルはダウンロードしない。
...
3. 注意事項
    1. ファイルを貼り付ける際、そのファイルがどういったものなのかを示す必要がある。
    2. テンプレートの使用  
        一部チャンネルでは独自のテンプレートを設置している場合がある。  
        これにより、有用な情報をを簡単に遡れるため、できるだけ従うこと。  
        また、それ以外の独自ルールを設けている場合もある。一度ピン止めを確認すること。


4. R18, R15制限に関して
    1. ゾーニング
        - **性器及び性行為**に類する画像の貼り付けは nfsw を指定したチャンネルで行うこと。
        - 年齢も広い範囲であるため、**R15相当** (女性に胸に関わる) 部分の露骨な表現に関しては **nsfw** とする。
        - 「性的目的で利用される製品」に関しても nsfw 指定とする。
        - **露骨な表現及び無修正を含む画像に関しては投稿を禁止**する。
        
        参考情報: 男女ともに水着で隠れる部分が見えている画像に関しては nsfw 指定とする。
    2. R18 URL の貼り付けに関して
        nsfw 指定チャンネル以外において、「R18であることを明記」した上で、 「URL から h を外し」「スポイラーをつけた上」で投稿することを許容する。  
        例: `[R18] ttp://hogefuga.com/eroi_video`  
        上記の3つを満たさない場合は削除対象となる。
    3. R18ゾーニング (グロテスク表現)
        参加者年齢が非常に広く、また、グロテスク表現に嫌悪感を抱く方がいることを考慮し、下記のように運用する。  
...
6. 入室  
    メンバーロールの付与は、**自己紹介 の 「アイサツ」へ投稿後**、メンバーロールをモデレータ以上が付与する。  
...
7. 権限
    ...
    2. モデレータ (青)
        以下の権限を保有し、**サーバの維持管理に協力**を行う。メンバーに追加して設定を行う。
        - 発言の削除  
        - チャンネルの追加  
        - ひとりごとチャンネルの権限追加削除  
        - ユーザのタイムアウト  
...
8. 招待リンク  
    招待リンクは下記の共通のリンクを利用する。  
    このリンクは予告なしで変更される場合がある。  
    https://discord.gg/7X4pX9Fjhb
...
9. 依頼
    ...
    - チャンネル名の変更
    - チャンネルの削除 (モデレーター以上のみ閲覧可能な archive へ移動させます)


10. 処罰
    モデレーター以上が必要とした場合、**緊急処置として発言権を停止**することがある。  
    本人から事情を聴いたうえで、必要に応じてモデレータ以上の会議において、タイムアウト、永久発言禁止、BAN等の処罰を行う。  
        注意: これは、 モデレータ 及び 管理者 に対し多大な迷惑をかける行為である。  
